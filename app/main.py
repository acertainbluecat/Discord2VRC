...
# TODO Change to indexed id instead of filename? guess it'd be faster
@app.get("/vrc/all/{fileName}")
async def findByFileName(fileName: int):
    image = await db.find_one(Image, Image.filename == fileName)
    if image:
        return RedirectResponse(url="/"+image.filepath)
    return RedirectResponse(url=placeholder)
